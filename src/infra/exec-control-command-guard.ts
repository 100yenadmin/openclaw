...
function collectPayloadsForArgv(
  ...
  for (const payloadText of shellPayloadTextsFromArgv(argv)) {
    ...
    const shellStatements = splitTopLevelShellStatements(payloadText);
    if (shellStatements.length > 1) {
      ...
      for (const statement of shellStatements) {
        ...
        const assignmentAdjusted = extractLeadingEnvAssignments(statementArgv, shellEnv);
        ...
        const statementContext: SearchGuardContext = {
          ...context,
          env: assignmentAdjusted.env ?? shellEnv,
        };
        payloads.push(
