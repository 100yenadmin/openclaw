...
- Data widgets are **thin re-implementations** against read RPCs resolved through the L3 controller (which may call the RPCs directly through the page's gateway client OR via `dashboard.data.read` — pick ONE path and document it; recommendation: direct client RPCs for builtins [they run trusted with the page's scopes], `dashboard.data.read` reserved for the custom-widget bridge). Do NOT import existing page view functions — they're welded to their pages' state.
